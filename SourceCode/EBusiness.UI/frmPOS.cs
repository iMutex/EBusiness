...
namespace EBusiness
{
    public partial class frmPOS : Form
    {
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            ...
            EBusiness.Reports.ReportsApp reportApp = new Reports.ReportsApp();
        ...
        {
            EBusiness.Reports.CurrentCashReport frm = new Reports.CurrentCashReport();
            frm.ShowDialog();
        }
        ...
        private void AddToGrid()
        {
            if (dgvAllItems.CurrentRow != null)
            {
                ...
                if (ItemId > 0)
                {
                    ...
                    DataTable dt = item.SelectItemById(ItemId.ToString());
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    if (isFirstRow)
                    {
                        ...
                        try
                        {
                            if (Convert.ToString(row["CustomerCode"]).ToUpper() == "CASH")
                            {
                                chkCash.Checked = true;
                                txtCashName.Text = Convert.ToString(row["CustomerName"]);
                                txtCashAddress.Text = Convert.ToString(row["CustomerAddress"]);
                            }
                            else
                            {
                            ...
                            txtPurpose.Text = "Return Receipt for Receipt No. " + ReceiptId.ToString();
                    ...
                    decimal discount = Convert.ToDecimal(row["ReceiptDetailDiscount"]);

                    decimal disc = price * (discount / 100);
                    decimal totaldiscount = Quantity * disc;// grandTotal - Quantity * disc;
                    decimal Total = Convert.ToDecimal(row["ReceiptDetailTotal"]);
                    long stock = Quantity;


                    AddRowToGrid(ItemId, code, name, packing, price, discount.ToString("#0.00"), Quantity, grandTotal, totaldiscount, Quantity);
        ...
        void Save(string CustomerId)
        {
            ...
            if (dgvItems.Rows.Count > 0)
            {
                ...
                long receiptId = item.SaveReceipt(
                    ...
                    txtPurpose.Text,
                    dt,
                    chkIsReturn.Checked, txtReturnReceiptNo.Text);
        ...
        void ResetReceipt()
        {
            ...
            chkCash.Checked = !chkCash.Checked;
            chkCash.Checked = !chkCash.Checked;
        ...
        {
            AllItemsDataTable = item.SelectAllStockActiveItem();
        ...
        private void txtReturnReceiptNo_TextChanged(object sender, EventArgs e)
        {

        }

        private void lblReturnReceiptNo_Click(object sender, EventArgs e)
        {
        ...
        private void cbCustomer_Leave(object sender, EventArgs e)
        {
            ...
            {
                if (!Common.IsCodeExists(code, Modules.Customer))
                {
                    MessageBox.Show("Customer does not exists OR is Inactive.", "Customer Not Found", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cbCustomer.Focus();
                }
                else
                {
